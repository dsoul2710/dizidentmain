...
import UserManager from "../../components/common/UserManager.jsx";
...
import GenericProviderPortalView from "../provider/GenericProviderPortalView.jsx";
...
export default function UnifiedDashboard({ user, onLogout }) {
  ...
  const navItems = useMemo(() => {
    ...
    // Clinical Schedules
    if (hasPermission(user, "APPOINTMENTS", "view") && user.role !== "PATIENT" && !isSuperAdmin) {
      items.push({ type: "group", label: "Schedule" });
    ...
    // Finance & billing
    if (hasPermission(user, "BILLING_FINANCE", "view") && user.role === "ORG") {
    ...
    // Inventory
    if (hasPermission(user, "INVENTORY", "view") && user.role === "ORG") {
  ...
  return (
    ...
    >
      <Routes>
        ...
        {hasPermission(user, "APPOINTMENTS", "view") && user?.role !== "PATIENT" && user?.role !== "SUPERADMIN" && user?.role !== "SUPER_ADMIN" && (
          <>
            <Route
              path="appointments"
              element={
        ...
        {hasPermission(user, "BILLING_FINANCE", "view") && user?.role === "ORG" && (
          <>
        ...
        {hasPermission(user, "INVENTORY", "view") && user?.role === "ORG" && (
