// src/components/AdultRaphaelChart.jsx
import React, { useEffect, useRef } from "react";

// Adult FDI order (must match your SVG tooth classes)
const ADULT_ORDER = [
...
// 🔴 IMPORTANT: Paste your full <svg>…</svg> from the HTML file here
// The example below only shows the beginning – you must replace the comment
// with the complete SVG you posted (including all <path>, <text>, etc).
const ADULT_SVG = `<svg xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" version="1.1" width="289.61084" height="370.54398">      
...
const REGION_GROUPS = {
  upperLeft:  ["11","12","13","14","15","16","17","18"],
  upperRight: ["21","22","23","24","25","26","27","28"],
  lowerLeft:  ["41","42","43","44","45","46","47","48"],
  lowerRight: ["31","32","33","34","35","36","37","38"],
};

export default function AdultRaphaelChart({ value = [], onChange }) {
  const containerRef = useRef(null);
  const toothMapRef = useRef({});
  const valueRef = useRef(value);
  const clickHandlerRef = useRef(null);

  // keep latest selection in a ref & repaint
  useEffect(() => {
    valueRef.current = value || [];
    repaintFromValue();
  }, [value]);

  const repaintFromValue = () => {
    const container = containerRef.current;
    if (!container) return;

    const toothMap = toothMapRef.current || {};
    const sel = new Set(valueRef.current || []);

    Object.keys(toothMap).forEach((num) => {
      const { paths = [], label } = toothMap[num];
      const selected = sel.has(num);

      paths.forEach((el) => {
        if (!el) return;
        el.style.stroke = selected ? "#1b5d46" : "#000000";
        el.style.strokeWidth = selected ? "2" : "1";
        el.style.fill = selected ? "#4ee1a0" : "none";
      });

      if (label) {
        label.style.fill = selected ? "#0b0c10" : "#000000";
  ...
  const resolveToothFromTarget = (target) => {
    if (!target) return null;
    const matchEl =
      typeof target.closest === "function"
        ? target.closest("[data-tooth]")
        : null;
    const raw = matchEl?.dataset?.tooth;
    if (!raw) return null;
    const trimmed = String(raw).trim();
    return ADULT_ORDER.includes(trimmed) ? trimmed : null;
  };
  ...
  const handleSvgClick = (event) => {
    ...
    const num = resolveToothFromTarget(event?.target);
    if (!num) return;
    toggleTooth(num);
  };
  ...
  const initSvg = () => {
    ...
    ADULT_ORDER.forEach((num) => {
      const paths = Array.from(svg.querySelectorAll(`.tooth-${num}`));
      const label = Array.from(svg.querySelectorAll("text")).find(
        (t) => (t.textContent || "").trim() === num
      );

      const isSelected = () => (valueRef.current || []).includes(num);
      ...
      const hoverIn = () => {
        if (!isSelected()) {
          paths.forEach((el) => {
            ...
            el.style.fill = "#fde68a";
          });
        }
      };

      const hoverOut = () => repaintFromValue();

      [...paths, label].forEach((el) => {
        if (!el) return;
        el.style.cursor = "pointer";
        el.dataset.tooth = num;
        el.addEventListener("mouseenter", hoverIn);
        el.addEventListener("mouseleave", hoverOut);
      });

      toothMap[num] = { paths, label, hoverIn, hoverOut };
    });

    toothMapRef.current = toothMap;
    if (!clickHandlerRef.current) {
      clickHandlerRef.current = handleSvgClick;
      svg.addEventListener("click", clickHandlerRef.current, true);
    }
    repaintFromValue();
  };

  // Mount: inject SVG once & wire listeners
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    container.innerHTML = ADULT_SVG; // insert raw SVG
    initSvg();

    return () => {
      const map = toothMapRef.current || {};
      Object.values(map).forEach(({ paths, label, hoverIn, hoverOut }) => {
        [...paths, label].forEach((el) => {
          if (!el) return;
      ...
      const svg = container.querySelector("svg");
      if (svg && clickHandlerRef.current) {
        svg.removeEventListener("click", clickHandlerRef.current, true);
      }
  ...
  // ---------- TOGGLE HELPERS (regions + full mouth) ----------

  const handleRegionToggle = (regionKey) => {
    const regionTeeth = REGION_GROUPS[regionKey];
    if (!regionTeeth) return;

    const currentSet = new Set(valueRef.current || []);
    const allSelected = regionTeeth.every((t) => currentSet.has(t));

    let next;
    if (allSelected) {
      // turn OFF → remove region teeth
      next = [...currentSet].filter((t) => !regionTeeth.includes(t));
    } else {
      // turn ON → add all region teeth
      next = Array.from(new Set([...currentSet, ...regionTeeth]));
    }
  ...
  const handleFullMouthToggle = () => {
    const currentSet = new Set(valueRef.current || []);
    const allSelected = ADULT_ORDER.every((t) => currentSet.has(t));

    let next;
    if (allSelected) {
      // full mouth was on → clear everything
      next = [];
    } else {
      // full mouth off → select all teeth
      next = [...ADULT_ORDER];
    }
  ...
  // derived checked state for switches (based on current value prop)
  const selectedSet = new Set(value || []);
  const upperLeftOn  = REGION_GROUPS.upperLeft.every((t) => selectedSet.has(t));
  const upperRightOn = REGION_GROUPS.upperRight.every((t) => selectedSet.has(t));
  const lowerLeftOn  = REGION_GROUPS.lowerLeft.every((t) => selectedSet.has(t));
  const lowerRightOn = REGION_GROUPS.lowerRight.every((t) => selectedSet.has(t));
  const fullMouthOn  = ADULT_ORDER.every((t) => selectedSet.has(t));

  return (
    <div>
      <div className="odo-diagram">
        {/* SVG container */}
        <div
          ref={containerRef}
          className="odo-chart"
        ...
        {/* Floating region toggles */}
        <button
          type="button"
          className={`exam-btn odo-float odo-ul ${upperLeftOn ? "active" : ""}`}
          onClick={() => handleRegionToggle("upperLeft")}
        >
          <span className="odo-btn-label">Upper Left</span>
          <span className="odo-btn-range">(11-18)</span>
        </button>

        <button
          type="button"
          className={`exam-btn odo-float odo-ur ${upperRightOn ? "active" : ""}`}
          onClick={() => handleRegionToggle("upperRight")}
        >
          <span className="odo-btn-label">Upper Right</span>
          <span className="odo-btn-range">(21-28)</span>
        </button>

        <button
          type="button"
          className={`exam-btn odo-float odo-ll ${lowerLeftOn ? "active" : ""}`}
          onClick={() => handleRegionToggle("lowerLeft")}
        >
          <span className="odo-btn-label">Lower Left</span>
          <span className="odo-btn-range">(41-48)</span>
        </button>

        <button
          type="button"
          className={`exam-btn odo-float odo-lr ${lowerRightOn ? "active" : ""}`}
          onClick={() => handleRegionToggle("lowerRight")}
        >
          <span className="odo-btn-label">Lower Right</span>
          <span className="odo-btn-range">(31-38)</span>
        </button>

        <button
          type="button"
          className={`exam-btn odo-float odo-center ${fullMouthOn ? "active" : ""}`}
          onClick={handleFullMouthToggle}
        >
          <span className="odo-btn-label">Full</span>
          <span className="odo-btn-range">Mouth</span>
        </button>
      </div>
